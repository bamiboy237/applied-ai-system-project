...
def test_review_context_dispatches_tool_calls(monkeypatch, capsys) -> None:
    ...
    monkeypatch.setattr(
        llm.registry,
        "execute",
        lambda name, args: Path("codereview/prompts.py").read_text(encoding="utf-8"),
    )
    ...
    assert fake_client.responses.calls[1]["input"] == [
        {
            ...
            "call_id": "call_1",
            "output": Path("codereview/prompts.py").read_text(encoding="utf-8"),
        }
    ]
    stderr = capsys.readouterr().err
...
def test_review_context_can_collect_agent_loop_logs(monkeypatch) -> None:
    fake_client = FakeClient(
        [
            (
                [
                ...
                make_response(response_id="resp_1", output_text="REVIEW:1: avoid print"),
            ),
        ]
    )
    logs: list[str] = []
    ...
    result = llm.review_context("## Target File\n1 | print('x')", log_sink=logs.append)

    assert result == CopilotResult(reviews={1: ["avoid print"]})
    assert "starting model turn 1" in logs
    assert "model is drafting the review" in logs
    assert "no tool calls requested; parsing final response" in logs
...
def test_review_file_builds_context(monkeypatch, tmp_path, capsys) -> None:
    ...
    monkeypatch.setattr(
        llm,
        "review_context",
        lambda context, message=llm.DEFAULT_TEACHER_MESSAGE, log_sink=None: CopilotResult(
