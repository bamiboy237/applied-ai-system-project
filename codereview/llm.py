...
def _log(message: str, log_sink: LogSink | None = None) -> None:
...
def _dispatch_tool_call(
    ...
    try:
        ...
        _log(
            f"running tool `{tool_call.name}` with args {_summarize(json.dumps(args, sort_keys=True))}",
            log_sink,
        )
        ...
        _log(_summarize(result), log_sink)
    except Exception as exc:
        result = f"Error running tool '{tool_call.name}': {exc}"
        _log(_summarize(result), log_sink)
    else:
        _log(f"tool `{tool_call.name}` returned {_summarize(result)}", log_sink)
...
def _stream_turn(
    *,
    ...
    _log(f"starting model turn {turn_number}", log_sink)
    saw_text = False
        ...
        for event in stream:
            if getattr(event, "type", None) == "response.created":
                ...
                if response is not None:
                    _log(f"response created: {response.id}", log_sink)
            elif getattr(event, "type", None) == "response.in_progress":
                _log(f"model turn {turn_number} in progress", log_sink)
            elif isinstance(event, ResponseOutputItemAddedEvent):
                item = event.item
                if isinstance(item, ResponseFunctionToolCall):
                    _log(f"model requested tool `{item.name}`", log_sink)
                ...
                _log("model is drafting the review", log_sink)
                saw_text = True
            elif getattr(event, "type", None) == "response.completed":
                _log(f"model turn {turn_number} completed", log_sink)
...
def assist_context(
    context: str,
    message: str = DEFAULT_TEACHER_MESSAGE,
    log_sink: LogSink | None = None,
) -> CopilotResult:
    ...
    for turn_number in range(1, MAX_TOOL_CALLS + 1):
        ...
        if not tool_calls:
            _log("no tool calls requested; parsing final response", log_sink)
            return parse_copilot_result(response.output_text)

        tool_outputs = [
            _dispatch_tool_call(tool_call, log_sink=log_sink)
    ...
    _log(
        f"reached max tool-call loop limit ({MAX_TOOL_CALLS}); parsing latest response",
        log_sink,
    )
    return parse_copilot_result(response.output_text)
...
def assist_file(
    ...
    _log(f"assisting {target_file}", log_sink)
    return assist_context(build_context(target_file), message=message, log_sink=log_sink)
...
def review_context(
    ...
    return assist_context(context, message=message, log_sink=log_sink)
...
def review_file(
    ...
    """Backward-compatible alias for the previous review-only API name."""
    _log(f"assisting {target_file}", log_sink)
    return review_context(build_context(target_file), message=message, log_sink=log_sink)
