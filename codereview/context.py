...
def is_context_candidate(file: Path, project_root: Path | None = None) -> bool:
    """Return whether a Python file should be eligible for model context."""
    root = project_root or _project_root()
    try:
        relative = file.resolve().relative_to(root)
    except ValueError:
        return False
    return file.suffix == ".py" and not any(
        part in EXCLUDED_CONTEXT_PARTS for part in relative.parts
    )


def iter_context_files(project_root: Path | None = None) -> list[Path]:
    """Return project Python files, excluding generated and dependency folders."""
    root = project_root or _project_root()
    return sorted(
        file
        for file in root.rglob("*.py")
        if is_context_candidate(file, project_root=root)
    )
...
def map_project_skeleton() -> list[str]:
    ...
    for file in iter_context_files(project_root):
        try:
